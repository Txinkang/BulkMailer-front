...
const getAssignDetails = async () => {
  try{
    if(props.active_name === 'supplier'){
      const requestData = {
        supplier_id: props.user_id,
        page_num: assignDetailsPagination.value.serverPage,
        page_size: assignDetailsPagination.value.serverPageSize
      }
      console.log('获取供应商分配详情请求数据：', requestData)
      const response = await supplierApi.assignSupplierDetails(requestData)
      if(response.code !== 200){
        clearCache()
        errorHandler.showError('获取供应商分配详情失败，请重试',response)
      }else{
        ...
        console.log('供应商分配详情响应数据：', response)
      }
    }else{
      const requestData = {
      ...
      console.log('获取客户分配详情请求数据：', requestData)
      const response = await customerApi.assignCustomerDetails(requestData)
      if(response.code !== 200){
        clearCache()
        errorHandler.showError('获取客户分配详情失败，请重试',response)
      }else{
        assignDetailsPagination.value.cachedData.set(assignDetailsPagination.value.serverPage, response.data.data)
        assignDetailsPagination.value.totalItems = response.data.total_items
        console.log('客户分配详情响应数据：', response)
      }
    }
  } catch (error) {
    clearCache()
    errorHandler.showError('获取客户、供应商分配详情失败，请刷新页面', error)
  }
}
// 清除缓存
const clearCache = () => {
