...
const emit = defineEmits(['update:modelValue', 'confirm', "clearSelection"])
...
const confirmAssign = async () => {
  try {
    ...
    if(props.active_tab === 'supplier'){
      const requestData = {
        supplier_id: props.selectedRows,
        belong_user_id: selectedUser.value.id
      }
      console.log('确定批量分配供应商请求数据：', requestData)
      const response = await supplierApi.allAssignSupplier(requestData)
      if(response.code !== 200){
        errorHandler.showError('批量分配供应商失败，请重试', response)
      }else{
        ElMessage.success('批量分配供应商成功')
        closeDialog()
        console.log('批量分配供应商成功',response)
      }
    }else if(props.active_tab === 'customer'){
      const requestData = {
        customer_id: props.selectedRows,
        belong_user_id: selectedUser.value.id
      }
      console.log('确定批量分配客户请求数据：', requestData)
      const response = await customerApi.allAssignCustomer(requestData)
      if(response.code !== 200){
        errorHandler.showError('批量分配客户失败，请重试', response)
      }else{
        ElMessage.success('批量分配客户成功')
        closeDialog()
        console.log('批量分配客户成功',response)
      }
    }
  } catch (error) {
    errorHandler.showError('批量分配失败，请重试', error)
  }
}
